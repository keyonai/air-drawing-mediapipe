...
Draw in the air using just your hand and a webcam. MediaPipe tracks 21 hand landmarks in real time to detect finger gestures — point to draw, peace sign to erase, fist to lift the pen.
...
| ✊ All fingers down | Lift pen (stop drawing) |
...
Hand gesture recognition is a key input modality for human-robot interaction. This project demonstrates real-time skeleton-based gesture classification using only an RGB camera — the same type of pipeline used in teleoperation interfaces and gesture-controlled robotic arms.
